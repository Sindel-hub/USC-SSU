...
From **Admin → User Directory**, open the student/officer account and choose **Reset password**. In the browser-only/Spark runtime, the administrator sends a Firebase password-recovery link to the account's registered institutional email. If the optional privileged Cloud Function is deployed, the same action can instead issue a one-time temporary password for the administrator to hand directly to the account owner. The old password is never displayed or stored in Firestore.
