...
function currentStudentUid() {
  return String(auth.currentUser?.uid || profile()?.uid || "").trim();
}
...
const CLASSIFICATION_DESCRIPTIONS = {
  "Student Level": "Handled primarily through student or USC support channels.",
  "Administrative Level": "Requires coordination with a university office, department, or administrator.",
  "Crisis Level": "Urgent safety, welfare, or serious-risk concern requiring immediate escalation."
};
function complaintClassification(value = '') {
  const level = clean(value);
  return COMPLAINT_CLASSIFICATIONS.includes(level) ? level : '';
}
function classificationClass(value = '') {
  const level = complaintClassification(value);
  if (level === 'Student Level') return 'student';
  if (level === 'Administrative Level') return 'administrative';
  if (level === 'Crisis Level') return 'crisis';
  return 'pending';
}
function classificationLabel(value = '') { return complaintClassification(value) || 'Pending Classification'; }
function classificationDescription(value = '') {
  const level = complaintClassification(value);
  return CLASSIFICATION_DESCRIPTIONS[level] || 'USC will assign this classification after reviewing your complaint.';
}

function complaintClassificationForRecord(record = {}) {
  const direct = complaintClassification(record?.classification);
  if (direct) return direct;
  const thread = Array.isArray(record?.thread) ? record.thread : [];
  for (let index = thread.length - 1; index >= 0; index -= 1) {
    const fromThread = complaintClassification(thread[index]?.classification);
...
function renderComplaintPreview() {
  ...
  host.innerHTML = `
    <div class="complaint-preview-box">
      ...
      <div class="complaint-preview-classification"><strong>USC Classification:</strong><span class="track-classification-pill ${classificationClass(item.classification)}">${escapeHtml(classificationLabel(item.classification))}</span></div>
...
function renderRecentActivity() {
  ...
  state.complaints.slice(0,3).forEach(item => {
    items.push({title: 'Complaint Updated', text: `Your complaint is ${clean(item.status,'Submitted')}${complaintClassification(item.classification) ? ` • ${complaintClassification(item.classification)}` : ''}.`, when: formatDateTime(item.updatedAt || item.createdAt)});
  });
...
function renderComplaintTable() {
  ...
  if (!state.complaints.length) {
    body.innerHTML = '<tr><td colspan="8">No complaints submitted yet.</td></tr>';
  ...
  body.innerHTML = state.complaints.map(item => `
    <tr>
      ...
      <td><span class="track-classification-pill ${classificationClass(item.classification)}">${escapeHtml(classificationLabel(item.classification))}</span></td>
...
function renderOfficerReplies(complaint) {
  ...
  const officerEntries = thread
    ...
    .filter(entry => clean(entry?.kind).toLowerCase() !== 'classification')
  ...
  if (!officerEntries.length) {
    host.innerHTML = `
      <div class="officer-reply-empty">
        ...
        <div><strong>No USC feedback yet</strong><span>After review, official USC feedback will appear here automatically. This complaint thread is one-way.</span></div>
      </div>`;
    return;
  }
  ...
  host.innerHTML = officerEntries.map((entry, index) => `
    <article class="officer-reply-message">
      ...
      <div class="officer-reply-content">
        <div class="officer-reply-meta"><strong>USC Feedback${officerEntries.length > 1 ? ` ${index + 1}` : ''}</strong><time>${escapeHtml(formatDateTime(entry?.at))}</time></div>
...
function openComplaintDetails(complaintId) {
  ...
  const classification = classificationLabel(complaint.classification);
  const classificationEl = document.getElementById('detailComplaintClassification');
  if (classificationEl) {
    classificationEl.textContent = classification;
    classificationEl.className = `detail-classification-pill ${classificationClass(complaint.classification)}`;
  }
  setText('detailComplaintClassificationNote', classificationDescription(complaint.classification));
...
function renderComplaintLoadError(error) {
  console.error('Student complaint tracklist listener error:', error);
  state.complaints = [];
  renderAll();
  const host = document.getElementById('complaintPreviewHost');
  if (host) {
    host.innerHTML = `<div class="track-empty-card"><p>Unable to load your complaint records right now. Please refresh the page or sign in again.</p></div>`;
  }
}

function listenComplaints() {
  const studentUid = currentStudentUid();
  if (!studentUid) return renderAll();
  const complaintsQuery = query(collection(db, 'complaints'), where('studentUid', '==', studentUid));
  onSnapshot(complaintsQuery, snapshot => {
    state.complaints = snapshot.docs.map(doc => normalizeComplaintRecord({ id: doc.id, ...doc.data() })).sort((a,b) => (toDate(b.updatedAt || b.createdAt)?.getTime() || 0) - (toDate(a.updatedAt || a.createdAt)?.getTime() || 0));
    renderAll();
  }, renderComplaintLoadError);
}

async function loadTracklistElectionContext() {
  const pointer = await getDoc(doc(db, 'election_config', 'current'));
  const electionId = clean(pointer.data()?.electionId);
  if (!pointer.exists() || !electionId) return null;
  const election = await getDoc(doc(db, 'elections', electionId));
  if (!election.exists()) return null;
  return { electionId, title: clean(election.data()?.title, 'USC Election') };
}
...
async function listenVotes() {
  ...
  if (!studentUid) return renderAll();
  try {
    const context = await loadTracklistElectionContext();
    if (!context?.electionId) {
      state.participation = [];
      renderAll();
      return;
    }
    const statusRef = doc(db, 'elections', context.electionId, 'voterStatus', studentUid);
