...
function getComplaintClassification(complaint = {}) {
  const direct = safeText(complaint?.classification);
  ...
  for (let index = thread.length - 1; index >= 0; index -= 1) {
    const fromThread = safeText(thread[index]?.classification);
...
function getLatestThreadEntry(complaint) {
  ...
  return [...thread].reverse().find((entry) =>
    safeText(entry?.by).toLowerCase() === "officer"
    && safeText(entry?.kind).toLowerCase() !== "classification"
    && !/^status updated from/i.test(safeText(entry?.message))
  ) || null;
}
...
function buildTracklistCard(complaint) {
  ...
  const classification = getComplaintClassification(complaint);
  ...
  const lastActor = lastEntry ? "USC" : "Pending";
  const lastMessage = truncateText(lastEntry?.message || "USC feedback will appear here after your complaint has been reviewed.", 150);
  const lastMessageTime = lastEntry ? formatDateTime(lastEntry?.at, "Recent") : "Awaiting review";
  ...
  return `
    <article class="track-item ${statusClass}">
      ...
      <div class="track-item-chips">
        ...
        <div class="track-item-chip"><i class="fa-solid fa-layer-group"></i><span>${escapeHtml(classification)}</span></div>
      </div>
      <div class="track-item-grid">
        <div class="track-mini-card"><small>Latest update</small><strong>${escapeHtml(latestUpdate)}</strong></div>
        <div class="track-mini-card"><small>USC classification</small><strong>${escapeHtml(classification)}</strong></div>
      </div>
      <div class="track-thread-preview ${lastEntry ? "officer" : "student"}">
        <div class="track-thread-head"><strong>${escapeHtml(lastEntry ? "USC feedback" : "Awaiting USC feedback")}</strong><span>${escapeHtml(lastMessageTime)}</span></div>
...
function startComplaintTracklistListener(studentProfile) {
  const studentUid = safeText(auth.currentUser?.uid || studentProfile?.uid);
  if (!studentUid) {
    renderTracklist([]);
    return;
  }
  ...
  return `
    <article class="event-item ${isHighlighted ? "highlighted" : ""}">
      ...
      <div class="event-copy">
        <h4>${escapeHtml(safeText(eventItem.title, eventItem.sourceType === "program" ? "Untitled Program" : "Untitled Event"))}</h4>
        <p>${escapeHtml(eventItem.sourceType === "program" ? `${safeText(eventItem.hostOrganization, "External partner")} · ${safeText(eventItem.venue, "Venue to be announced")}` : safeText(eventItem.venue, "Venue to be announced"))}</p>
    ...
    list.innerHTML = `
      <div class="events-empty">
        <div class="events-empty-card">
          ...
          <h3>No events or programs scheduled</h3>
          <p>University events and outside-university programs will appear here automatically.</p>
        </div>
      </div>
    `;
    return;
...
function startDashboardEventsListener() {
  const rebuild = () => {
    dashboardEventState.events = [...dashboardEventState.sources.events, ...dashboardEventState.sources.programs]
  ...
  const bind = (collectionName, sourceType) => {
    onSnapshot(
      query(collection(db, collectionName), orderBy("eventDate", "asc")),
      (snapshot) => {
        dashboardEventState.sources[collectionName] = snapshot.docs
          .map((docItem) => ({ id: docItem.id, ...docItem.data(), sourceType }))
          .filter((eventItem) => safeText(eventItem.status, "Published") === "Published");
        rebuild();
      },
      (error) => {
        console.error(`Dashboard ${collectionName} listener error:`, error);
        dashboardEventState.sources[collectionName] = [];
        rebuild();
      }
    );
  };
