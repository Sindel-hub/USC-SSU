...
const feeds = { announcements: [], events: [], programs: [], complaints: [], election: null, application: null };
...
const COMPLAINT_CLASSIFICATION_LEVELS = ["Student Level", "Administrative Level", "Crisis Level"];
function complaintClassification(record = {}) {
  const direct = clean(record?.classification);
  if (COMPLAINT_CLASSIFICATION_LEVELS.includes(direct)) return direct;
  const thread = Array.isArray(record?.thread) ? record.thread : [];
  for (let index = thread.length - 1; index >= 0; index -= 1) {
    const fromThread = clean(thread[index]?.classification);
    if (COMPLAINT_CLASSIFICATION_LEVELS.includes(fromThread)) return fromThread;
  }
  return "";
}
function toDate(v) {
...
function eventRegistrationHint(eventItem = {}) {
  const mode = clean(eventItem.registrationMode, "none").toLowerCase();
  if (!["internal", "external"].includes(mode)) return "";
  const deadline = toDate(eventItem.registrationDeadline);
  if (!deadline) return "";
  if (deadline.getTime() <= Date.now()) return " • Registration closed";
  return ` • Registration closes ${relativeTime(deadline)}`;
}
...
function buildItems() {
  ...
  const now=Date.now();
  feeds.events.filter(e=>e.reminderEnabled !== false && eventDate(e.eventDate)?.getTime() >= now-86400000).slice(0,3).forEach(e=>list.push(item(`event:${e.id}`,"event","Event Reminder",`${clean(e.title,"USC event")} • ${clean(e.venue,"See event details")}${eventRegistrationHint(e)}`,e.registrationDeadline||e.eventDate,"fa-calendar-days",dashboardHref("events.html"))));
  feeds.programs.filter(e=>e.reminderEnabled !== false && eventDate(e.eventDate)?.getTime() >= now-86400000).slice(0,3).forEach(e=>list.push(item(`program:${e.id}`,"event","External Program Reminder",`${clean(e.title,"External program")} • ${clean(e.hostOrganization,"Partner organization")} • ${clean(e.venue,"See program details")}${eventRegistrationHint(e)}`,e.registrationDeadline||e.eventDate,"fa-earth-americas",dashboardHref("events.html"))));
  feeds.complaints.slice(0,3).forEach(c=>{
    const classification=complaintClassification(c);
    const feedbackCount=Array.isArray(c.thread)?c.thread.filter(entry=>clean(entry?.by).toLowerCase()==="officer" && clean(entry?.kind).toLowerCase()!=="classification" && !/^status updated from/i.test(clean(entry?.message))).length:0;
    const classificationText=classification?` • ${classification}`:"";
    const feedbackText=feedbackCount?" USC feedback is available in your Tracklist.":"";
    list.push(item(
      `complaint:${c.id}:${clean(c.status)}:${classification || "pending"}:${feedbackCount}`,
      "complaint",
      "Complaint Update",
      `Your complaint ${clean(c.complaintRef,c.id)} is now ${clean(c.status,"updated")}${classificationText}.${feedbackText}`,
      c.updatedAt||c.createdAt,
      "fa-comments",
      dashboardHref("tracklist.html")
    ));
  });
  if (feeds.election) {
...
function listen() {
  ...
  onSnapshot(query(collection(db,"programs"),orderBy("eventDate","asc"),limit(8)),s=>{feeds.programs=s.docs.map(d=>({id:d.id,...d.data()}));render();},()=>{});
  const studentUid=clean(auth.currentUser?.uid || p?.uid);
  if (studentUid) onSnapshot(query(collection(db,"complaints"),where("studentUid","==",studentUid)),s=>{feeds.complaints=s.docs.map(d=>({id:d.id,...d.data()})).sort((a,b)=>(toDate(b.updatedAt||b.createdAt)?.getTime()||0)-(toDate(a.updatedAt||a.createdAt)?.getTime()||0));render();},error=>{console.warn("Complaint notification feed unavailable:",error);});
