...
export function complaintAttachmentMeta(complaint = {}) {
  return {
    ...
    name: clean(complaint.attachmentName || complaint.fileName),
...
async function getLegacySupabase() {
  // Supabase is only needed for attachments created by older project revisions.
  // Loading it lazily prevents a third-party CDN failure from stopping the entire
  // student tracklist module (including complaint records) from starting.
...
async function loadLegacySupabase(meta) {
  ...
  const supabase = await getLegacySupabase();
