...
import { effectiveOfficerPermissions, normalizeOfficerPermissions, permissionsForOfficerPosition } from "./officer-permissions.js";

let supabaseClientPromise = null;
async function getSupabaseClient() {
  if (!supabaseClientPromise) {
    supabaseClientPromise = import("../supabase/supabase-config.js").then((module) => module.supabase);
  }
...
async function browserAdminUpdateUser(data = {}) {
  ...
  const candidacyApproved = role === "student" && standingActive && (typeof data.candidacyApproved === "boolean" ? data.candidacyApproved : old.candidacyApproved === true);
  ...
  const officerPermissions = role === "officer"
    ? normalizeOfficerPermissions(
        Array.isArray(data.officerPermissions)
          ? data.officerPermissions
          : (Array.isArray(old.officerPermissions) ? old.officerPermissions : permissionsForOfficerPosition(officePosition)),
        { legacyFullAccess: false }
      )
    : [];
  const officerPermissionSource = role === "officer"
    ? String(data.officerPermissionSource || old.officerPermissionSource || (Array.isArray(data.officerPermissions) ? "custom" : "position-default")).trim().toLowerCase()
    : "";
...
function complaintClassificationFromRecord(record = {}) {
  const allowed = ["Student Level", "Administrative Level", "Crisis Level"];
  const direct = String(record?.classification || "").trim();
  if (allowed.includes(direct)) return direct;
  const thread = Array.isArray(record?.thread) ? record.thread : [];
  for (let index = thread.length - 1; index >= 0; index -= 1) {
    const value = String(thread[index]?.classification || "").trim();
...
async function browserUpdateComplaintStatus(data = {}) {
  const { user, profile, role } = await requireBrowserPermission("complaints.review");

  const complaintId = String(data.complaintId || "").trim();
  const requestedStatus = String(data.status || "").trim();
  const requestedClassification = String(data.classification || "").trim();
  const previousStatus = String(data.previousStatus || "Submitted").trim() || "Submitted";
  const previousClassification = String(data.previousClassification || "").trim();
  const complaintRefText = String(data.complaintRef || complaintId).trim() || complaintId;
  const allowedStatuses = ["Submitted", "Under Review", "In Progress", "Resolved", "Closed"];
  const allowedClassifications = ["Student Level", "Administrative Level", "Crisis Level"];

  if (!complaintId) throw new Error("Complaint ID is required.");
  if (requestedStatus && !allowedStatuses.includes(requestedStatus)) throw new Error("Invalid complaint status.");
  if (requestedClassification && !allowedClassifications.includes(requestedClassification)) throw new Error("Invalid complaint classification.");

  const nextStatus = requestedStatus || previousStatus;
  const nextClassification = requestedClassification || previousClassification;
  if (["In Progress", "Resolved", "Closed"].includes(nextStatus) && !allowedClassifications.includes(nextClassification)) {
    throw new Error("Assign a complaint classification before moving the case beyond review.");
  }

  const statusChanged = nextStatus !== previousStatus;
  const classificationChanged = Boolean(requestedClassification && requestedClassification !== previousClassification);
  if (statusChanged && !browserHasOfficerPermission(profile, role, "complaints.status")) throw new Error("Your assigned USC office cannot update complaint status.");
  if (classificationChanged && !browserHasOfficerPermission(profile, role, "complaints.classify")) throw new Error("Your assigned USC office cannot classify complaints.");
  if (!statusChanged && !classificationChanged) {
    return { ok: true, unchanged: true, classification: nextClassification || "" };
  }

  const complaintRef = doc(db, "complaints", complaintId);

  // First try the current rules format. This deliberately updates ONLY the
  // complaint document. Audit logging is best-effort afterwards so a log rule
  // can never roll back a valid USC review.
  const modernPatch = { updatedAt: serverTimestamp() };
  if (statusChanged) modernPatch.status = nextStatus;
  if (classificationChanged) modernPatch.classification = requestedClassification;

  try {
    await updateDoc(complaintRef, modernPatch);

    try {
      await addDoc(collection(db, "complaint_case_logs"), {
        actorUid: user.uid,
        actorEmail: user.email || "",
        action: classificationChanged ? "COMPLAINT_REVIEW_UPDATED" : "COMPLAINT_STATUS_UPDATED",
        complaintId,
        complaintRef: complaintRefText,
        statusBefore: previousStatus,
        statusAfter: nextStatus,
        classificationBefore: previousClassification || "Pending Classification",
        classificationAfter: nextClassification || "Pending Classification",
        source: "browser-officer-runtime",
        createdAt: serverTimestamp()
      });
    } catch (auditError) {
      console.warn("Complaint review saved, but its audit entry was blocked by older Firestore rules.", auditError);
    }

    return { ok: true, previousStatus, status: nextStatus, classification: nextClassification || "" };
  } catch (error) {
    if (!isFirestorePermissionDenied(error)) throw error;
  }

  // Compatibility path for the original deployed capstone rules. Those rules
  // allow status/updatedAt/thread but do not allow a root-level classification.
  // Put the classification in the immutable USC review thread instead. All
  // complaint UIs already resolve classification from this thread when needed.
  const legacyPatch = { updatedAt: serverTimestamp() };
  if (statusChanged) legacyPatch.status = nextStatus;
  if (classificationChanged) {
    legacyPatch.thread = arrayUnion({
      by: "Officer",
      kind: "classification",
      actorUid: user.uid,
      classification: requestedClassification,
      message: `USC classification assigned: ${requestedClassification}.`,
      at: Timestamp.now()
    });
  }

  try {
    await updateDoc(complaintRef, legacyPatch);
  } catch (legacyError) {
    if (isFirestorePermissionDenied(legacyError)) {
      throw new Error(
        "USC can view this complaint but Firebase is blocking complaint updates for this account. " +
        "Refresh the page and sign in again. If it continues, publish the included firestore.rules so the officer account has complaint-update permission."
      );
    }
    throw legacyError;
  }

  // Old deployed rules may reject newer audit fields. Keep this completely
  // separate from the complaint write and ignore permission failures.
  try {
    await addDoc(collection(db, "complaint_case_logs"), {
      actorUid: user.uid,
      actorEmail: user.email || "",
    ...
    console.warn("Complaint review saved in compatibility mode; audit log was not permitted.", auditError);
  }
...
async function browserDeleteComplaintCase(data = {}) {
  const { user } = await requireBrowserPermission("complaints.status");
...
async function browserSendAdminPasswordResetEmail(data = {}) {
  const { user: adminUser } = await requireBrowserRole(["admin"]);
  const uid = String(data?.uid || "").trim();
  if (!uid) throw new Error("Target user UID is required.");
  if (uid === adminUser.uid) throw new Error("Use the administrator account settings to change the administrator password.");

  const profileSnap = await getDoc(doc(db, "users", uid));
  if (!profileSnap.exists()) throw new Error("User profile not found.");
  const profile = profileSnap.data() || {};
  const role = String(profile.role || "").trim().toLowerCase();
  if (!["student", "officer"].includes(role)) throw new Error("Only student and officer passwords can be recovered from the User Directory.");

  const email = String(profile.authEmail || profile.institutionalEmail || profile.email || "").trim().toLowerCase();
  if (!email) throw new Error("This account has no registered authentication email for password recovery.");

  await sendPasswordResetEmail(auth, email);

  try {
    await browserRecordAudit({
      action: "USER_PASSWORD_RESET_EMAIL_SENT",
      target: `users/${uid}`,
      details: {
        targetRole: role,
        targetEmail: email,
        reason: String(data?.reason || "Password reset requested by account holder.").trim().slice(0, 500),
        recoveryMode: "firebase-email"
      }
    });
  } catch (auditError) {
    console.warn("Password reset email sent, but the browser audit entry could not be saved.", auditError);
  }
  ...
  return {
    ...
    uid,
    role,
    fullName: String(profile.fullName || "User").trim(),
    studentId: String(profile.studentId || "").trim(),
    institutionalEmail: email,
    warning: "A password reset link was sent because direct temporary-password issuance requires the deployed Admin password-reset Cloud Function."
  };
}
...
function browserHasOfficerPermission(profile = {}, role = "", permission = "") {
  if (role === "admin") return true;
  if (role !== "officer") return false;
  return effectiveOfficerPermissions({ ...profile, role }, { legacyFullAccess: !Array.isArray(profile.officerPermissions) }).includes(permission);
}

async function requireBrowserPermission(permission, roles = ["officer", "admin"]) {
  const context = await requireBrowserRole(roles);
  if (!browserHasOfficerPermission(context.profile, context.role, permission)) {
    throw new Error(`Your assigned USC office does not have permission for ${permission.replaceAll(".", " ")}.`);
  }
  return context;
}
...
async function browserStoreFile(file, kind) {
  ...
  const allowedKinds = new Set(["event-media", "program-media", "announcement-media", "candidate-photo", "candidate-document"]);
  ...
  if (["event-media", "program-media", "announcement-media", "candidate-photo"].includes(kind) && !isImage) {
  ...
  const visibility = ["event-media", "program-media", "announcement-media"].includes(kind) ? "public" : "private";
  const batch = writeBatch(db);
  batch.set(ref, {
...
async function browserComplaintReply(data = {}) {
  const { user, profile, role } = await requireBrowserPermission("complaints.feedback");
  const complaintId = String(data.complaintId || "").trim();
  const feedback = String(data.feedback || data.reply || "").trim().slice(0,4000);
  const requestedStatus = String(data.status || "").trim();
  const previousStatus = String(data.previousStatus || "Submitted").trim() || "Submitted";
  const classification = String(data.classification || data.previousClassification || "").trim();
  const allowedClassifications = ["Student Level", "Administrative Level", "Crisis Level"];
  const allowedStatuses = ["Submitted", "Under Review", "In Progress", "Resolved", "Closed"];

  if (!complaintId || !feedback) throw new Error("Complaint ID and USC feedback are required.");
  if (!allowedClassifications.includes(classification)) throw new Error("Assign a valid complaint classification before sending feedback.");
  if (classification !== String(data.previousClassification || "").trim() && !browserHasOfficerPermission(profile, role, "complaints.classify")) {
    throw new Error("Your assigned USC office cannot change complaint classification.");
  }

  let effectiveStatus = allowedStatuses.includes(requestedStatus) ? requestedStatus : previousStatus;
  if (effectiveStatus === "Submitted") effectiveStatus = "Under Review";
  ...
  // Current rules format.
  try {
    await updateDoc(ref, {
      status: effectiveStatus,
      classification,
      updatedAt: serverTimestamp(),
      thread: arrayUnion(feedbackEntry)
    });
    return { updated: true, status: effectiveStatus, classification, feedbackAdded: true, replyAdded: true };
  } catch (error) {
    if (!isFirestorePermissionDenied(error)) throw error;
  }

  // Original rules compatibility format: classification lives in the appended
  // USC feedback entry instead of the complaint root.
  try {
    await updateDoc(ref, {
      status: effectiveStatus,
      updatedAt: serverTimestamp(),
    ...
    if (isFirestorePermissionDenied(legacyError)) {
      throw new Error(
        "Firebase is blocking USC feedback updates for this account. Refresh and sign in again; if it persists, publish the included firestore.rules."
      );
    }
    throw legacyError;
  }

  return { updated: true, status: effectiveStatus, classification, feedbackAdded: true, replyAdded: true, compatibilityMode: true };
}

async function browserSaveElectionSchedule(data = {}, emergency = false) {
  const { user } = emergency ? await requireBrowserRole(["admin"]) : await requireBrowserPermission("elections.view");
  const electionId = String(data.electionId || "").trim().toLowerCase();
  if (!/^[a-z0-9][a-z0-9-]{2,79}$/.test(electionId)) {
    throw new Error("Use a permanent Election ID containing lowercase letters, numbers, and hyphens.");
  }

  const schedule = normalizedSchedule(data.schedule || {});
  const electionRef = doc(db, "elections", electionId);
  const pointerRef = doc(db, "election_config", "current");
  const [existing, current] = await Promise.all([getDoc(electionRef), getDoc(pointerRef)]);
  ...
  if (before?.finalized === true || before?.archived === true || before?.resultsPublished === true) {
    throw new Error("A finalized, published-result, or archived election cannot be rescheduled.");
  }
  ...
  if (previousId && previousId !== electionId) {
    ...
    if (previous.exists() && previous.data()?.archived !== true) {
      throw new Error(`Archive the current election (${previousId}) before switching to a new Election ID.`);
    }
  }

  const reason = String(data.reason || "").trim().slice(0, 500);
  if (emergency && reason.length < 15) {
    throw new Error("A specific emergency schedule reason of at least 15 characters is required.");
  }

  const title = String(data.title || before?.title || "USC General Election").trim().slice(0, 120);
  const lifecycle = lifecycleFromSchedule({
    ...(before || {}),
    ...schedule,
    finalized: false,
  ...
  const patch = {
    ...
    title,
    ...schedule,
    candidateReviewComplete: before?.candidateReviewComplete === true,
    finalized: false,
    resultsPublished: false,
    archived: false,
    ...
    ...(emergency ? {
      emergencyChangeAt: serverTimestamp(),
      emergencyChangeByUid: user.uid
    } : {})
  };

  const batch = writeBatch(db);
  batch.set(electionRef, patch, { merge: true });
  batch.set(pointerRef, { electionId, updatedAt: serverTimestamp() }, { merge: true });
  batch.set(doc(collection(db, "audit_logs")), {
    actorUid: user.uid,
    actorEmail: user.email || "",
    action: emergency ? "ELECTION_EMERGENCY_SCHEDULE_UPDATE" : "ELECTION_SCHEDULE_SAVE",
    target: `elections/${electionId}`,
    source: "browser-election-runtime",
  ...
  return {
    electionId,
    lifecycle,
    emergencyChangeRecorded: emergency
  };
}


async function browserDeleteElectionSchedule(data = {}) {
  const { user } = await requireBrowserPermission("elections.view");
  const electionId = String(data.electionId || "").trim().toLowerCase();
  if (!electionId) throw new Error("No election schedule was selected.");
  const electionRef = doc(db, "elections", electionId);
  const pointerRef = doc(db, "election_config", "current");
  const [existing, pointer] = await Promise.all([getDoc(electionRef), getDoc(pointerRef)]);
  if (!existing.exists()) throw new Error("The election schedule no longer exists.");
  const before = existing.data();
  if (before.finalized === true || before.resultsPublished === true || before.archived === true) {
    throw new Error("A finalized, published-result, or archived election schedule cannot be deleted.");
  }
  const batch = writeBatch(db);
  batch.set(electionRef, {
    scheduleDeleted: true,
    scheduleDeletedAt: serverTimestamp(),
    scheduleDeletedByUid: user.uid,
    lifecycle: "Draft",
    updatedAt: serverTimestamp()
  }, { merge: true });
  if (pointer.exists() && String(pointer.data()?.electionId || "") === electionId) batch.delete(pointerRef);
  batch.set(doc(collection(db, "audit_logs")), {
    actorUid: user.uid,
    actorEmail: user.email || "",
    action: "ELECTION_SCHEDULE_DELETE",
    target: `elections/${electionId}`,
    source: "browser-election-runtime",
    details: { preservedElectionRecord: true },
    createdAt: serverTimestamp()
  });
  await batch.commit();
  return { electionId, deleted: true };
}

async function browserStartRosterImport(data = {}) {
  const { user } = await requireBrowserPermission("elections.roster");
...
function cleanRosterRow(raw = {}) {
  ...
  const eligible = isActiveStudentStanding(studentStanding, raw.enrollmentStatus);
...
async function browserFinalizeRosterImport(data = {}) {
  await requireBrowserPermission("elections.roster");
  const electionId = String(data.electionId || "").trim();
  const importId = String(data.importId || "").trim();
  ...
  batch.set(doc(db,"elections",electionId,"turnout","public"), { eligibleVoters: eligibleVoterCount, ballotsCast: 0, departmentVotes:{}, updatedAt: serverTimestamp() }, { merge:true });
...
async function browserSubmitCandidateApplication(data = {}) {
  ...
  if (profile.schoolProvisioned !== true) throw new Error("Your school-provided student account is required for candidate registration.");
  if (profile.candidacyApproved !== true) throw new Error("The System Administrator has not approved this account for candidacy registration.");
...
async function browserReviewCandidate(data = {}) {
  const { user } = await requireBrowserPermission("elections.review");
...
async function browserGetElectionDepartmentTurnout(data = {}) {
  await requireBrowserPermission("elections.view");
  const context = await browserElectionContext();
  if (data?.electionId && String(data.electionId) !== String(context.electionId)) throw new Error("Department turnout is available only for the active election.");
  const turnoutSnap = await getDoc(doc(db, "elections", context.electionId, "turnout", "public"));
  const turnoutData = turnoutSnap.exists() ? turnoutSnap.data() : {};
  const departmentVotes = turnoutData.departmentVotes && typeof turnoutData.departmentVotes === "object" ? turnoutData.departmentVotes : {};
  const departments = Object.entries(departmentVotes)
    .map(([department, voted]) => ({ department: String(department || "Unspecified department"), voted: Math.max(0, Number(voted || 0)) }))
    .filter((row) => row.voted > 0)
    .sort((left, right) => right.voted - left.voted || left.department.localeCompare(right.department));
  const totalVoters = departments.reduce((sum, row) => sum + row.voted, 0);
  return {
    electionId: context.electionId,
    totalVoters,
    departments: departments.map((row) => ({ ...row, share: totalVoters > 0 ? Number(((row.voted / totalVoters) * 100).toFixed(2)) : 0 }))
  };
}

async function browserSubmitBallot(data = {}) {
  const { user, profile } = await requireBrowserRole(["student"]);
  if (profile.schoolProvisioned !== true) throw new Error("A school-provided student account is required to vote.");
  const context = await browserElectionContext();
  ...
  const studentId = String(profile.studentId||"").trim();
  const voterCollege = String(profile.college || "").trim();
  for (const position of ELECTION_POSITIONS) {
    ...
    if (position === "Department Representative" && String(candidate.data().department||"") !== voterCollege) throw new Error("You may only vote for your verified college representative.");
  }
  ...
  await runTransaction(db, async (tx) => {
    ...
    tx.set(statusRef,{ hasVoted:true, studentId, college:voterCollege, votedAt:serverTimestamp(), receiptReference });
    const turnoutDepartment = voterCollege || "Unspecified department";
    tx.set(doc(db,"elections",context.electionId,"turnout","public"), { ballotsCast:increment(1), eligibleVoters:Number(context.eligibleVoterCount||0), departmentVotes:{ [turnoutDepartment]:increment(1) }, updatedAt:serverTimestamp() }, { merge:true });
  });
  return { recorded:true, receiptReference };
}

async function browserFinalizeElection() {
  const { user, profile, role } = await requireBrowserPermission("elections.results");
...
async function callBrowserMode(name, data) {
  ...
  if (name === "getElectionDepartmentTurnout") return browserGetElectionDepartmentTurnout(data);
  ...
  if (name === "updateComplaintCase" && String(data?.feedback || data?.reply || "").trim()) return browserComplaintReply(data);
  ...
  if (name === "deleteElectionSchedule") return browserDeleteElectionSchedule(data);
...
export async function callSecure(name, data = {}) {
  try {
    ...
    if (name === "adminResetUserPassword" && globalThis.USC_FREE_SPARK_MODE === true) {
      try {
        return await browserSendAdminPasswordResetEmail(data);
      } catch (fallbackError) {
        const wrappedFallback = new Error(cleanError(fallbackError, "Unable to send the password recovery link."));
        wrappedFallback.code = fallbackError?.code || "unknown";
        wrappedFallback.cause = fallbackError;
