...
import { callStudentEmailService } from "./supabase-email-service.js";
import {
  doc,
  getDoc,
  updateDoc,
...
function createDrawer() {
  ...
  drawer.innerHTML = `
    ...
    <section id="gmailSettings" class="gmail-settings usc-profile-gmail" aria-label="Personal Gmail notifications and recovery">
      <div class="gmail-card-head">
        <div class="gmail-card-title-wrap"><span class="gmail-card-icon"><i class="fa-solid fa-envelope"></i></span><div><strong>Recovery & Email Alerts</strong><span>Personal Gmail</span></div></div>
        <small class="gmail-self-service-badge">Self-service</small>
      </div>
      <div class="gmail-status-card" data-state="loading"><span class="gmail-status-icon"><i class="fa-solid fa-circle-notch fa-spin"></i></span><span class="gmail-status-copy"><strong>Loading Gmail settings</strong><small>Please wait while your saved recovery email is checked.</small></span></div>
    </section>
  ...
  if (!document.getElementById("uscEmailSettingsStyles")) {
...
function fillDrawer(profile) {
  ...
  if (drawer) drawer.dataset.profileRole = profile.role || "student";
...
function openDrawer(trigger) {
  ...
  const gmailSettings = drawer.querySelector("#gmailSettings");
  if (["student", "officer"].includes(currentProfile.role) && gmailSettings) {
    import("./email-settings.js")
      .then(({ initializeEmailSettings }) => initializeEmailSettings(gmailSettings))
      .catch((error) => {
        console.error("Gmail settings load error:", error);
        gmailSettings.innerHTML = '<div class="gmail-settings-head"><strong>Personal Gmail</strong><small>Supabase email</small></div><p role="alert">Email settings could not be loaded. Deploy the Supabase Edge Function and refresh this page.</p>';
      });
  }
...
async function notifyPasswordChangedByEmail() {
  if (!["student", "officer"].includes(currentProfile?.role || "") || !auth.currentUser) return false;
  try {
    const response = await callStudentEmailService("password_changed");
    return response?.sent === true;
  } catch (error) {
    console.warn("Supabase password security email could not be sent:", error);
    return false;
  }
}
...
async function changeOwnPassword() {
  ...
  try {
    ...
    const alertStarted = await notifyPasswordChangedByEmail();
    setPasswordMessage(
      alertStarted
        ? "Password changed successfully. Supabase sent a security alert to your verified Gmail."
        : "Password changed successfully. Connect and verify a Gmail above to enable Supabase security alerts and password recovery.",
      "success"
    );
  } catch (error) {
...
function bindDrawer(trigger, overlay, drawer) {
  ...
  drawer.addEventListener("keydown", (event) => {
    ...
    const focusable = [...drawer.querySelectorAll('button, input:not([type="hidden"]), summary, a[href], select, textarea')].filter((el) => !el.matches(':disabled') && el.getClientRects().length > 0);
...
function initialize() {
  ...
  import("./settings-manager.js")
    .then(({ initializeSettingsManager }) => initializeSettingsManager({ role: currentProfile.role }))
    .catch((error) => console.warn("Settings manager load error:", error));
