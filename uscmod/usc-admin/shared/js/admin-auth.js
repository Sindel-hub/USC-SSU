...
function normalizeProfile(raw = {}, user = null, claims = {}) {
  ...
  return {
    ...
    officerPermissionsConfigured: Array.isArray(raw.officerPermissions),
    officerPermissionSource: String(raw.officerPermissionSource || "").trim(),
    officerPermissions: Array.isArray(raw.officerPermissions) ? raw.officerPermissions.map((item) => String(item || "").trim()).filter(Boolean) : undefined
  };
}
function save(profile) { sessionStorage.setItem("activeSession","true"); sessionStorage.setItem("studentProfile",JSON.stringify(profile)); try { window.dispatchEvent(new CustomEvent("usc-officer-profile-updated", { detail: profile })); } catch {} }
