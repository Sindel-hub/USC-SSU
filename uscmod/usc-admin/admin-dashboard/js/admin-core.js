...
import { effectiveOfficerPermissions, normalizeOfficerPermissions, permissionsForOfficerPosition } from "../../../shared/officer-permissions.js?v=programs-1";
import {
  addDoc,
  collection,
  doc,
...
export function normalizeUser(docSnap) {
  ...
  return {
    ...
    candidacyApproved: data.candidacyApproved === true,
    ...
    college: String(data.college || data.department || "").trim(),
    ...
    officerPermissionsConfigured: Array.isArray(data.officerPermissions),
    storedOfficerPermissions: Array.isArray(data.officerPermissions) ? normalizeOfficerPermissions(data.officerPermissions, { legacyFullAccess: false }) : [],
    officerPermissionSource: String(data.officerPermissionSource || "").trim(),
    officerPermissions: role === "officer"
      ? effectiveOfficerPermissions({ ...data, role, officePosition: String(data.officePosition || "").trim() }, { legacyFullAccess: true })
      : [],
    adminNotes: String(data.adminNotes || "").trim(),
    createdAtMs: Number(data.createdAtMs || data.createdAt?.toMillis?.() || 0),
    updatedAtMs: Number(data.updatedAtMs || data.updatedAt?.toMillis?.() || 0),
    roleEffectiveDate: normalizeDateInputValue(data.roleEffectiveDate),
    roleEffectiveAtMs: Number(data.roleEffectiveAtMs || 0),
    lastLoginAtMs: Number(data.lastLoginAtMs || data.lastLoginAt?.toMillis?.() || 0),
    lastActivityAtMs: Number(data.lastActivityAtMs || data.lastActivityAt?.toMillis?.() || 0),
...
export function getFilteredUsers(users, filters = {}) {
  ...
  const filtered = users.filter((user) => {
    ...
    return name.includes(queryText) || studentId.includes(queryText);
  });
...
function buildUserUpdatePayload(user, source = {}) {
  ...
  const requestedCandidacyApproved = protectedAdmin ? false : (typeof source.candidacyApproved === "boolean" ? source.candidacyApproved : user.candidacyApproved === true);
  const nextCandidacyApproved = nextRole === "student" && standingActive ? requestedCandidacyApproved : false;
  ...
  const requestedOfficerPermissions = source.officerPermissions !== undefined
    ? source.officerPermissions
    : (user.officerPermissionsConfigured
      ? user.officerPermissions
      : (nextRole === "officer" ? permissionsForOfficerPosition(nextOfficePosition) : []));
  const nextOfficerPermissions = nextRole === "officer"
    ? normalizeOfficerPermissions(requestedOfficerPermissions, { legacyFullAccess: false })
    : [];
  const nextOfficerPermissionSource = nextRole === "officer"
    ? String(source.officerPermissionSource || (source.officerPermissions !== undefined ? "custom" : user.officerPermissionSource || "position-default")).trim().toLowerCase()
    : "";
  ...
  return {
    ...
    nextEffectiveDate,
    nextNotes,
    nextOfficerPermissions,
    nextOfficerPermissionSource,
    changeReason
  };
}
...
function summarizeChanges(user, payload) {
  ...
  if (user.isVerifiedStudent !== payload.nextVerified) parts.push(`school verification ${user.isVerifiedStudent ? "verified" : "unverified"} → ${payload.nextVerified ? "verified" : "unverified"}`);
  if ((user.candidacyApproved === true) !== payload.nextCandidacyApproved) parts.push(`candidacy ${payload.nextCandidacyApproved ? "approved" : "not approved"}`);
  ...
  const currentPermissions = user.role === "officer"
    ? normalizeOfficerPermissions(user.officerPermissions, { legacyFullAccess: user.officerPermissionsConfigured !== true })
    : [];
  if (JSON.stringify([...currentPermissions].sort()) !== JSON.stringify([...payload.nextOfficerPermissions].sort())) parts.push("officer permissions updated");
  if ((user.officerPermissionSource || "") !== payload.nextOfficerPermissionSource) parts.push("officer permission defaults synchronized");
  if ((user.adminNotes || "") !== payload.nextNotes) parts.push("notes updated");
  return parts;
}

function requiresReason(changeParts) {
  return changeParts.some((part) => part.startsWith("role") || part.startsWith("status") || part.startsWith("active") || part.startsWith("standing") || part.startsWith("candidacy") || part.startsWith("officer permissions"));
}
...
export async function updateSingleUser(user, source = {}) {
  ...
  await callSecure("adminUpdateUser", {
    ...
    officerPermissions: payload.nextOfficerPermissions,
    officerPermissionSource: payload.nextOfficerPermissionSource,
    accountStatus: payload.nextStatus,
    isVerifiedStudent: payload.nextVerified,
    candidacyApproved: payload.nextCandidacyApproved,
...
export function exportUsersCsv(users, complaintSummaryByUid, voteSummaryByStudentId, filename) {
  ...
  const rows = users.map((user) => {
    ...
    return [
      ...
      user.officePosition || "",
      user.role === "officer" ? normalizeOfficerPermissions(user.officerPermissions, { legacyFullAccess: user.officerPermissionsConfigured !== true }).join(" | ") : "",
...
let analyticsBackfillInFlight = null;
let analyticsBackfillSignature = "";

async function backfillPreFeatureStudentAnalytics(users = []) {
  const students = users.filter((user) =>
    normalizeRoleValue(user.role) === "student"
    && String(user.uid || "").trim()
    && String(user.college || "").trim()
  );
  if (!students.length) return;

  const signature = students
    .map((user) => `${user.uid}:${user.lastLoginAtMs || 0}:${user.college}`)
    .sort()
    .join("|");
  if (signature && signature === analyticsBackfillSignature) return;
  if (analyticsBackfillInFlight) return analyticsBackfillInFlight;

  analyticsBackfillInFlight = (async () => {
    try {
      const existingSnapshot = await getDocs(collection(db, "student_signin_stats"));
      const existingIds = new Set(existingSnapshot.docs.map((item) => item.id));
      const missing = students.filter((user) => !existingIds.has(user.uid));
      if (!missing.length) {
        analyticsBackfillSignature = signature;
        return;
      }
      ...
      for (let offset = 0; offset < missing.length; offset += 400) {
        ...
        chunk.forEach((user) => {
          const historicalLoginMs = Math.max(0, Number(user.lastLoginAtMs || 0));
          const historicalLoginAt = historicalLoginMs ? Timestamp.fromMillis(historicalLoginMs) : null;
          batch.set(doc(db, "student_signin_stats", user.uid), {
            studentUid: user.uid,
            department: String(user.college || "").trim(),
            program: String(user.program || "").trim(),
            signInCount: historicalLoginAt ? 1 : 0,
            firstSignInAt: historicalLoginAt,
            lastSignInAt: historicalLoginAt,
            accountCreatedAtMs: Math.max(0, Number(user.createdAtMs || 0)),
            legacyBackfill: true,
      ...
      analyticsBackfillSignature = signature;
      console.info(`Student sign-in analytics backfilled for ${missing.length} pre-feature account(s).`);
    } catch (error) {
      console.warn("Unable to backfill pre-feature student analytics:", error);
    } finally {
      analyticsBackfillInFlight = null;
    }
  })();

  return analyticsBackfillInFlight;
}

export function subscribeUsers(callback) {
  return onSnapshot(collection(db, "users"), (snapshot) => {
    const normalized = snapshot.docs.map(normalizeUser);
    callback(normalized);
    void backfillPreFeatureStudentAnalytics(normalized);
