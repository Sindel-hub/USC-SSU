...
const OFFICER_PERMISSION_KEYS = new Set([
  "dashboard.view", "elections.view", "elections.schedule", "elections.roster", "elections.review", "elections.results",
  "complaints.view", "complaints.review", "complaints.classify", "complaints.feedback", "complaints.status", "complaints.report",
  "bulletin.manage", "events.manage", "programs.manage", "organization.view", "reports.generate"
]);
const ALL_OFFICER_PERMISSION_KEYS = [...OFFICER_PERMISSION_KEYS];
const DEFAULT_OFFICER_PERMISSION_KEYS = ["dashboard.view", "organization.view"];

function normalizeOfficerPermissionList(value, legacyFullAccess = true) {
  if (!Array.isArray(value)) return legacyFullAccess ? [...ALL_OFFICER_PERMISSION_KEYS] : [...DEFAULT_OFFICER_PERMISSION_KEYS];
  const out = [...new Set(value.map((item) => cleanText(item, 80)).filter((item) => OFFICER_PERMISSION_KEYS.has(item)))];
  if (!out.includes("dashboard.view")) out.unshift("dashboard.view");
  return out;
}

const OFFICER_POSITION_PERMISSION_PRESETS = Object.freeze({
  "President": [
    "dashboard.view",
    "complaints.view", "complaints.review", "complaints.classify", "complaints.feedback", "complaints.status", "complaints.report",
    "bulletin.manage", "events.manage", "programs.manage", "organization.view", "reports.generate"
  ...
  "Vice President": [
    "dashboard.view",
    "elections.view", "elections.roster", "elections.review", "elections.results",
    "bulletin.manage", "events.manage", "organization.view", "reports.generate"
  ],
  "Secretary": ["dashboard.view", "bulletin.manage", "events.manage", "programs.manage", "organization.view", "reports.generate"],
  "Treasurer": ["dashboard.view", "organization.view", "reports.generate"],
  "Auditor": ["dashboard.view", "organization.view", "reports.generate"],
  "Public Relations Officer (PRO)": ["dashboard.view", "bulletin.manage", "events.manage", "programs.manage", "organization.view"],
  "Business Manager": ["dashboard.view", "events.manage", "programs.manage", "organization.view", "reports.generate"],
  "Sgt. at Arms": ["dashboard.view", "events.manage", "organization.view"],
  "Department Representative": ["dashboard.view", "complaints.view", "complaints.review", "organization.view"]
});

function permissionsForOfficerPosition(position = "") {
  const normalized = cleanText(position, 120).toLowerCase();
  for (const [name, permissions] of Object.entries(OFFICER_POSITION_PERMISSION_PRESETS)) {
    if (name.toLowerCase() === normalized) return [...permissions];
  }
  return [...DEFAULT_OFFICER_PERMISSION_KEYS];
}

function isBaselineOfficerPermissionList(value) {
  if (!Array.isArray(value)) return false;
  const normalized = normalizeOfficerPermissionList(value, false);
  return normalized.length === DEFAULT_OFFICER_PERMISSION_KEYS.length
    && DEFAULT_OFFICER_PERMISSION_KEYS.every((item) => normalized.includes(item));
}

function effectiveOfficerPermissionList(profile = {}) {
  const stored = Array.isArray(profile.officerPermissions)
    ? normalizeOfficerPermissionList(profile.officerPermissions, false)
    : null;
  const source = cleanText(profile.officerPermissionSource, 40).toLowerCase();
  const preset = permissionsForOfficerPosition(profile.officePosition);
  const hasKnownPreset = Object.keys(OFFICER_POSITION_PERMISSION_PRESETS)
    .some((name) => name.toLowerCase() === cleanText(profile.officePosition, 120).toLowerCase());
  if (source === "custom" && stored) return stored;
  if (hasKnownPreset && (source === "position-default" || !stored || isBaselineOfficerPermissionList(stored))) return preset;
  if (stored) return stored;
  if (hasKnownPreset) return preset;
  return normalizeOfficerPermissionList(undefined, true);
}

async function requireOfficerPermission(request, permission) {
  const profile = await requireActiveRole(request, ["officer", "admin"]);
  if (roleOf(request) === "admin") return profile;
  const permissions = effectiveOfficerPermissionList(profile);
  if (!permissions.includes(permission)) {
    throw new HttpsError("permission-denied", `Your assigned USC office does not have permission for ${permission.replaceAll(".", " ")}.`);
  }
  return profile;
}
...
function sanitizeMasterRow(row) {
  ...
  return {
    ...
    eligible: enrollmentIsActive(row.enrollmentStatus || "Enrolled"),
...
exports.getElectionDepartmentTurnout = onCall(callableOptions(), async (request) => {
  await requireOfficerPermission(request, "elections.view");
  const { electionId } = await currentElection();
  const requestedElectionId = cleanText(request.data?.electionId, 160);
  if (requestedElectionId && requestedElectionId !== electionId) {
    throw new HttpsError("failed-precondition", "Department turnout is available only for the active election.");
  }
  const statusSnap = await db.collection(`elections/${electionId}/voterStatus`).where("hasVoted", "==", true).get();
  const counts = new Map();
  for (const statusDoc of statusSnap.docs) {
    const department = cleanText(statusDoc.get("college"), 160) || "Unspecified department";
    counts.set(department, (counts.get(department) || 0) + 1);
  }
  const totalVoters = statusSnap.size;
  const departments = [...counts.entries()]
    .map(([department, voted]) => ({
      department,
      voted,
      share: totalVoters > 0 ? Number(((voted / totalVoters) * 100).toFixed(2)) : 0
    }))
    .sort((left, right) => right.voted - left.voted || left.department.localeCompare(right.department));
  return { electionId, totalVoters, departments };
});
...
exports.provisionSchoolAccounts = onCall(callableOptions({ timeoutSeconds: 540, memory: "512MiB" }), async (request) => {
  await requireOfficerPermission(request, "elections.roster");
...
exports.deleteElectionSchedule = onCall(callableOptions(), async (request) => {
  await requireOfficerPermission(request, "elections.view");
  requireRecentAuth(request);
  const requestedId = cleanText(request.data?.electionId, 80).toLowerCase();
  const pointerRef = db.doc("election_config/current");
  const pointerSnap = await pointerRef.get();
  const electionId = requestedId || (pointerSnap.exists ? cleanText(pointerSnap.get("electionId"), 80) : "");
  if (!electionId) throw new HttpsError("failed-precondition", "No active election schedule exists.");
  const ref = db.doc(`elections/${electionId}`);
  const beforeSnap = await ref.get();
  if (!beforeSnap.exists) throw new HttpsError("not-found", "Election schedule not found.");
  const before = beforeSnap.data();
  if (before.finalized === true || before.resultsPublished === true || before.archived === true) {
    throw new HttpsError("failed-precondition", "A finalized, published-result, or archived election schedule cannot be deleted.");
  }
  const batch = db.batch();
  batch.set(ref, {
    scheduleDeleted: true,
    scheduleDeletedAt: FieldValue.serverTimestamp(),
    scheduleDeletedByUid: request.auth.uid,
    lifecycle: "Draft",
    updatedAt: FieldValue.serverTimestamp()
  }, { merge: true });
  if (pointerSnap.exists && cleanText(pointerSnap.get("electionId"), 80) === electionId) batch.delete(pointerRef);
  batch.set(db.collection("audit_logs").doc(), auditData(request, "ELECTION_SCHEDULE_DELETE", ref.path, before, { scheduleDeleted: true, activePointerRemoved: true }));
  await batch.commit();
...
exports.submitCandidateApplication = onCall(callableOptions(), async (request) => {
  const activeStudent = await requireActiveRole(request, ["student"]);
  const { electionId, data: election } = await currentElection();
  const nowMs = Date.now();
  ...
  if (!userSnap.exists || user.accountStatus !== "approved" || user.isActive === false || user.schoolProvisioned !== true) {
    throw new HttpsError("permission-denied", "Your school-provided student account is not active for candidate registration.");
  }
  if (user.candidacyApproved !== true) {
    throw new HttpsError("permission-denied", "The System Administrator has not approved this account for candidacy registration.");
  }
...
exports.reviewCandidateApplication = onCall(callableOptions({ secrets: [SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY] }), async (request) => {
  await requireOfficerPermission(request, "elections.review");
  requireRecentAuth(request);
  const { electionId, data: election } = await currentElection();
  const nowMs = Date.now();
...
exports.submitAnonymousBallot = onCall(callableOptions(), async (request) => {
  await requireActiveRole(request, ["student"]);
  ...
  throw new HttpsError(
    ...
    const user = userSnap.data();
    if (user.role !== "student" || user.accountStatus !== "approved" || user.isActive === false || user.schoolProvisioned !== true) throw new HttpsError("permission-denied", "Your active school-provided student account is required to vote.");
    const studentId = normalizeStudentId(user.studentId);
    ...
    const voterCollege = cleanText(user.college, 160);
    ...
    for (const [position, , snap] of candidatePairs) {
      ...
      if (position === "Department Representative" && cleanText(snap.get("department"), 160) !== voterCollege) {
    ...
    tx.set(db.doc(`elections/${electionId}/turnout/public`), {
      ...
      departmentVotes: { [turnoutDepartment]: FieldValue.increment(1) },
      updatedAt: FieldValue.serverTimestamp()
    }, { merge: true });
    for (const [position, , snap] of candidatePairs) {
...
exports.finalizeElection = onCall(callableOptions(), async (request) => {
  await requireOfficerPermission(request, "elections.results");
...
exports.adminUpdateUser = onCall(callableOptions(), async (request) => {
  ...
  const officerPermissions = role === "officer"
    ? normalizeOfficerPermissionList(
        Array.isArray(request.data?.officerPermissions)
          ? request.data.officerPermissions
          : (Array.isArray(before.officerPermissions) ? before.officerPermissions : permissionsForOfficerPosition(officePosition)),
        false
      )
    : [];
  const officerPermissionSource = role === "officer"
    ? cleanText(request.data?.officerPermissionSource || before.officerPermissionSource || (Array.isArray(request.data?.officerPermissions) ? "custom" : "position-default"), 40).toLowerCase()
    : "";
  const requestedVerifiedStudent = role === "student"
    ...
    : false;
  const candidacyApproved = role === "student"
    ? (typeof request.data?.candidacyApproved === "boolean" ? request.data.candidacyApproved : before.candidacyApproved === true)
    : false;
  let targetAuth;
  try {
  ...
  try {
    ...
    { role: before.role, accountStatus: before.accountStatus, officePosition: before.officePosition, officerPermissions: before.officerPermissions || null, officerPermissionSource: before.officerPermissionSource || "", adminNotes: before.adminNotes || "", isVerifiedStudent: before.isVerifiedStudent === true, candidacyApproved: before.candidacyApproved === true },
    { role, accountStatus, officePosition, officerPermissions, officerPermissionSource, canvasser, adminNotes, roleEffectiveDate, isVerifiedStudent: requestedVerifiedStudent, candidacyApproved },
    { reason: changeReason });
  ...
  return { updated: true, role, accountStatus, officePosition, officerPermissions, officerPermissionSource, canvasser, isVerifiedStudent: requestedVerifiedStudent, candidacyApproved };
});



exports.adminResetUserPassword = onCall(callableOptions(), async (request) => {
  await requireActiveRole(request, ["admin"]);
  requireRecentAuth(request);

  const uid = cleanText(request.data?.uid, 128);
  const reason = cleanText(request.data?.reason || "Password reset requested by account holder.", 500);
  if (!uid) throw new HttpsError("invalid-argument", "User UID is required.");
  if (uid === request.auth.uid) {
    throw new HttpsError("failed-precondition", "Use the administrator's own account settings to change the administrator password.");
  }
  ...
  const role = cleanText(profile.role, 20).toLowerCase();
  if (!['student', 'officer'].includes(role)) {
    throw new HttpsError("failed-precondition", "Only student and officer passwords can be reset from the User Directory.");
  }
  ...
  try {
    ...
    if (error?.code === 'auth/user-not-found') {
      throw new HttpsError("failed-precondition", "This profile has no matching Firebase Authentication account.");
    }
    throw error;
  }
  ...
  try {
    ...
    console.error('adminResetUserPassword auth update failed', { uid, code: error?.code, message: error?.message });
    throw new HttpsError("internal", "The temporary password could not be issued. Check Cloud Functions logs and retry.");
  }
  ...
  try {
    ...
    console.error('adminResetUserPassword token revoke failed', { uid, code: error?.code, message: error?.message });
    warnings.push('Existing sessions could not be revoked immediately.');
  }

  try {
    await profileRef.set({
      passwordResetIssuedAt: FieldValue.serverTimestamp(),
      passwordResetIssuedAtMs: Date.now(),
      passwordResetIssuedByUid: request.auth.uid,
      passwordResetIssuedByEmail: normalizeEmail(request.auth?.token?.email),
      updatedAt: FieldValue.serverTimestamp(),
      updatedAtMs: Date.now()
    }, { merge: true });
  } catch (error) {
    console.error('adminResetUserPassword profile metadata failed', { uid, code: error?.code, message: error?.message });
    warnings.push('Password reset metadata could not be saved.');
  }

  try {
    await audit(request, "USER_PASSWORD_RESET", profileRef.path,
      { passwordResetIssued: false },
      { passwordResetIssued: true },
      { reason, targetRole: role, targetEmail: normalizeEmail(authUser.email || profile.institutionalEmail || profile.email) }
    );
  } catch (error) {
    console.error('adminResetUserPassword audit failed', { uid, code: error?.code, message: error?.message });
  ...
  return {
    ...
    uid,
    role,
    fullName: cleanText(profile.fullName || authUser.displayName || "User", 120),
    studentId: cleanText(profile.studentId, 30),
    institutionalEmail: normalizeEmail(profile.institutionalEmail || profile.email || authUser.email),
    temporaryPassword,
...
exports.updateComplaintCase = onCall(callableOptions(), async (request) => {
  await requireOfficerPermission(request, "complaints.review");
  const complaintId = cleanText(request.data?.complaintId, 160);
  const nextStatus = cleanText(request.data?.status, 40);
  const classification = cleanText(request.data?.classification, 60);
  const feedback = cleanText(request.data?.feedback || request.data?.reply, 4000);
  const allowedStatuses = new Set(["Submitted", "Under Review", "In Progress", "Resolved", "Closed"]);
  const allowedClassifications = new Set(["Student Level", "Administrative Level", "Crisis Level"]);
  ...
  if (classification && !allowedClassifications.has(classification)) throw new HttpsError("invalid-argument", "Unsupported complaint classification.");
  if (!nextStatus && !classification && !feedback) throw new HttpsError("invalid-argument", "Provide a status, classification, or USC feedback update.");
  if (nextStatus) await requireOfficerPermission(request, "complaints.status");
  if (classification) await requireOfficerPermission(request, "complaints.classify");
  if (feedback) await requireOfficerPermission(request, "complaints.feedback");
  ...
  await db.runTransaction(async (tx) => {
    ...
    const previousClassification = cleanText(before.classification || "", 60);
    let effectiveStatus = nextStatus || previousStatus;
    const effectiveClassification = classification || previousClassification;

    if (feedback && effectiveStatus === "Submitted") effectiveStatus = "Under Review";
    if (["In Progress", "Resolved", "Closed"].includes(effectiveStatus) && !allowedClassifications.has(effectiveClassification)) {
      throw new HttpsError("failed-precondition", "Assign a complaint classification before moving the case beyond review.");
    }
    if (feedback && !allowedClassifications.has(effectiveClassification)) {
      throw new HttpsError("failed-precondition", "Assign a complaint classification before providing USC feedback.");
    }

    const threadEntries = [];
    if (nextStatus && effectiveStatus !== previousStatus) {
      threadEntries.push({ by: "Officer", kind: "status", message: `Status updated from ${previousStatus} to ${effectiveStatus}.`, at: Timestamp.now() });
    }
    if (feedback) threadEntries.push({ by: "Officer", kind: "feedback", message: feedback, at: Timestamp.now() });
    ...
    tx.set(db.collection("audit_logs").doc(), auditData(request, "COMPLAINT_CASE_UPDATE", ref.path,
      { status: previousStatus, classification: previousClassification || "Pending Classification" },
      { status: effectiveStatus, classification: effectiveClassification || "Pending Classification", feedbackAdded: Boolean(feedback) }));
  });
  return { updated: true, status: nextStatus || null, classification: classification || null, feedbackAdded: Boolean(feedback), replyAdded: Boolean(feedback) };
});
...
exports.createUploadTicket = onCall(callableOptions({ secrets: [SUPABASE_URL, SUPABASE_SERVICE_ROLE_KEY] }), async (request) => {
  ...
  if (["candidate-photo", "candidate-document"].includes(kind)) {
  ...
  } else if (["announcement-media", "event-media", "program-media"].includes(kind)) {
    await requireActiveRole(request, ["officer", "admin"]);
    ...
    const folder = kind === "announcement-media" ? "announcements" : (kind === "program-media" ? "programs" : "events");
