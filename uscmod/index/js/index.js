...
import { openSupabaseRecoveryPage } from "../../shared/supabase-email-service.js";
...
async function recordStudentSignIn(profile, user) {
  const role = clean(profile?.role || "student").toLowerCase();
  if (role !== "student" || !user?.uid) return;

  const department = clean(profile?.college || profile?.department);
  const program = clean(profile?.program);
  if (!department) return;

  const statsRef = doc(db, "student_signin_stats", user.uid);
  try {
    await runTransaction(db, async (transaction) => {
      ...
      const currentCount = Math.max(0, Number(current.signInCount || 0));
      const update = {
        department,
        program,
      ...
      if (currentCount === 0 || !current.firstSignInAt) {
        update.firstSignInAt = serverTimestamp();
      }
      transaction.update(statsRef, update);
    });
  } catch (error) {
    // Analytics must never prevent a valid student from entering the portal.
    console.warn("Unable to record student sign-in analytics:", error);
  }
}
...
function bindSignin() {
  dom.signinForm?.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      if (session.role === "student") await recordStudentSignIn({ ...profile, role }, user);
